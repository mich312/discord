...
- **Circle overview** — picking a circle lands on its overview page: a
  customizable cover (blurb + pinned links) plus a live directory of rooms
  and voice tables. Admins edit it, joiners inherit it via the encrypted
  metadata rebroadcast, and it rides the encrypted backup — the relay
  never learns what a circle says about itself.
