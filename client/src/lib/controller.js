...
export class Controller {
  ...
  async boot() {
    ...
    const state = await this.db.kvGet('mlsState');
  ...
  async createIdentity(name) {
    ...
    this.me = result.name;
    await this.persistState(result.state);
    return new Uint8Array(await this.crypto('exportIdentity'));
  }

  /** Onboarding path B: restore from an unwrapped recovery bundle. */
