...
impl PgStore {
    ...
    async fn migrate(&self) -> Result<(), StoreError> {
        ...
        sqlx::raw_sql(
            ...
            -- CREATE TABLE IF NOT EXISTS plus ADD COLUMN IF NOT EXISTS has
            -- been enough and a downgrade happened to work by accident. The
            -- first destructive change breaks that, and without a recorded
            -- version an operator cannot tell which shape their database is
            -- in. Recording it now is cheap; retrofitting it after a bad
            -- upgrade is not.
        ...
        // silently operating on it is how a rollback corrupts data. Older is
        // fine — the batch above brings it forward.
        let found: Option<i32> = sqlx::query_scalar("SELECT version FROM schema_version WHERE id = 1")
        ...
        if let Some(found) = found {
            if found > SCHEMA_VERSION {
                return Err(StoreError::Backend(format!(
                    "database is at schema version {found}, but this relay only understands \
                     {SCHEMA_VERSION}. It was written by a newer build; upgrade the relay rather \
...
/// could no longer operate correctly against the new shape. Additive
/// changes (new table, new nullable column) do not need a bump — the
/// IF NOT EXISTS batch handles those in either direction.
